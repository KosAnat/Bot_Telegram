...
async def high_count(message: types.Message, state: FSMContext):
    ...
    for game in working_with_the_api.low_high(query=data['high_metacritic'], count=data['high_count']):
        await bot.send_message(message.from_user.id, game)
    await message.reply('Поиск завершен')
    await state.finish()
...
async def low_count(message: types.Message, state: FSMContext):
    ...
    for game in working_with_the_api.low_high(query=data['low_metacritic'], count=data['low_count'], flag=False):
        await bot.send_message(message.from_user.id, game)
    await message.reply('Поиск завершен')
    await state.finish()
...
async def range_game_count(message: types.Message, state: FSMContext):
    ...
    for game in working_with_the_api.range_game(query=data['range_metacritic'], count=data['range_game_count']):
        await bot.send_message(message.from_user.id, game)
    await message.reply('Поиск завершен')
    await state.finish()
...
async def history_command_handler(message: types.Message):
    ...
    c = conn.cursor()
    c.execute('SELECT query FROM history WHERE user_id=? ORDER BY created_at DESC LIMIT 10', (message.from_user.id,))
    rows = c.fetchall()
    conn.close()
...
async def helper(message: types.Message):
    ...
    await message.answer(
        ...
        'Команда <b>/help</b>:\n'
        'Данная команда выдает информацию о всех командах', parse_mode='html')
...
def register_handers_client(dp: Dispatcher):
    ...
    dp.register_message_handler(full_game, commands=['custom'], state=None)
    dp.register_message_handler(high, commands=['high'], state=None)
    dp.register_message_handler(low, commands=['low'], state=None)
    dp.register_message_handler(range, commands=['range'], state=None)
    dp.register_message_handler(history_command_handler, commands=['history'])
    dp.register_message_handler(helper, commands=['help'])
    dp.register_message_handler(the_сomands_does_not_exist)

    dp.register_message_handler(search_and_send_game, state=FSM_all_game.searching_game)

    dp.register_message_handler(high_metacritic, state=FSM_all_game.high_1)
    dp.register_message_handler(high_count, state=FSM_all_game.high_2)

    dp.register_message_handler(low_metacritic, state=FSM_all_game.low_1)
    dp.register_message_handler(low_count, state=FSM_all_game.low_2)

    dp.register_message_handler(range_metacritic, state=FSM_all_game.range_game_1)
    dp.register_message_handler(range_game_count, state=FSM_all_game.range_game_2)

    dp.register_message_handler(info_game, state=FSM_all_game.info_game)
